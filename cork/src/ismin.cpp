/*
    ismin.cpp, v 1.2 2010/11/09 by Marisa Thoma
   ...
   You should have received a copy of the GNU General Public License
    ...
    Modifications: bug fixes for equality tests (since now most fields are 
     actually used)
*/
...
{
	if (DFS_CODE.size() == 1) {
		if (DFS_CODE.back().fromlabel <= DFS_CODE.back().tolabel)  
  			return (true);
		else
			return (false);
	}
...
bool gSpan::project_is_min (Projected &projected)
{
...

	{
      ...
      unsigned int back_position = DFS_CODE_IS_MIN.size()-1;
      if (DFS_CODE[back_position].from != DFS_CODE_IS_MIN[back_position].from
        || DFS_CODE[back_position].to != DFS_CODE_IS_MIN[back_position].to
        || DFS_CODE[back_position].elabel != DFS_CODE_IS_MIN[back_position].elabel)
        return false;
...

	{
      ...
      if (DFS_CODE[back_position].from != DFS_CODE_IS_MIN[back_position].from
        || DFS_CODE[back_position].to != DFS_CODE_IS_MIN[back_position].to
        || DFS_CODE[back_position].elabel != DFS_CODE_IS_MIN[back_position].elabel
        || DFS_CODE[back_position].tolabel != DFS_CODE_IS_MIN[back_position].tolabel)
        return false;
			return project_is_min (tolabel->second);
		}
	}
